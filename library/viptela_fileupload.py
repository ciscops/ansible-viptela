...
def run_module():
    ...
    url = 'https://{0}/api/config/vm_lifecycle/images?deep'.format(viptela.params['host'])
    response = viptela.request(url, method='GET')
    viptela.result['data'] = response
    ...
    if viptela.params['state'] == 'present':
        if viptela.params['name'] not in images_dict:
            try:
            ...
            payload = {'image': {}}
            payload['image']['name'] = viptela.params['name']
            payload['image']['src'] = 'file://{0}/{1}.tar.gz'.format(viptela.params['dest'], viptela.params['name'])

            url = 'https://{0}/api/config/vm_lifecycle/images'.format(viptela.params['host'])
            response = viptela.request(url, method='POST', payload=json.dumps(payload))
            viptela.result['changed'] = True
    else:
        if viptela.params['name'] in images_dict:
            # Delete the image
            url = 'https://{0}/api/config/vm_lifecycle/images/image/{1}'.format(viptela.params['host'], viptela.params['name'])
            response = viptela.request(url, 'DELETE')
            viptela.result['changed'] = True
        else:
            viptela.result['changed'] = False
        # Delete the file
        # payload = {
        #     'image': { 'name': '{0}.tar.gz'.format(viptela.params['name']) }
        # }
        #
        # url = 'https://{0}/api/operations/system/file-delete/file'.format(viptela.params['host'], viptela.params['name'])
        # response = viptela.request(url, method='POST', payload=json.dumps(payload))
        #
        # if viptela.status == 204:
        #     viptela.result['changed'] = True
